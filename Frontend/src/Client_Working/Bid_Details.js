...
export default function Bid_Details() {
  ...
  useEffect(() => {
    setTimeout(() => {
      const data = { projectId: _id }
      ApiServices.clientBidDetails(data)
        .then((res) => {
        ...
        .catch((err) => {
          console.log("Error Fetcing Bids", err)
          setLoad(false)
        })
    }, 3000)
  }, [])
    ...
    <div id="carouselExampleRide" className="carousel slide" data-bs-ride="carousel">
      ...
      <div className="container-fluid bg-light py-5">
        <div className="container py-5">
          <h1 className="display-4 mb-4 text-center">Bids</h1>
          {load ? (
              ...
              {bids?.map((el, index) => (
                <div className="col-12 col-sm-6 col-md-4 col-lg-3" key={index}>
                  <div className="card h-100">
                    <img
                      src={ el?.projectId.attachmnent}
                      className="card-img-top"
                      alt="attachment"
                      style={{ height: '150px', objectFit: 'cover' }}
                    />
                    <div className="card-body">
                      <p className="card-text"><strong>Name:</strong> {el?.bdeId.name}</p>
...
{/* <p className="card-text"><strong>Category:</strong> {el?.bdeId.preferenceId}</p> */}