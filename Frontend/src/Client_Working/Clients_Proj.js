...
export default function Clients_Proj() {
    const [projects,setProjects] = useState([])
    const [load,setLoad] = useState(true)
    const params = useParams()
    const _id = sessionStorage.getItem("clientId")

    useEffect(()=>{
        setTimeout(()=>{
            const data = {clientId: _id}
            ApiServices.clientsAllProjects(data)
            .then((res)=>{
  ...
  return (
    ...
    <div className="container-fluid bg-light py-5">
            <div className="container py-5">
                <h1 className="display-4 mb-4 text-center">Client's Projects</h1>
                {load ? (
